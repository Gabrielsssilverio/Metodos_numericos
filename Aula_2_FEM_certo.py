...
'''''
gmsh.initialize()
gmsh.model.add('Example 1')
gmsh.model.occ.addRectangle(0, 0, 0, 1, 1, tag=20)
gmsh.model.occ.synchronize()
gmsh.model.addPhysicalGroup(2, [20])
gmsh.model.mesh.generate(3)
gmsh.write('example_01.msh')
gmsh.write('example_01.vtk')
...
#Exemplo 2
gmsh.initialize()
gmsh.model.add('Example 2')
gmsh.model.occ.addRectangle(0, 0, 0, 1, 1, tag=20)
gmsh.model.occ.synchronize()
gmsh.model.addPhysicalGroup(2, [20])
gmsh.model.mesh.generate(3)
gmsh.model.mesh.refine()
gmsh.write('example_02.msh')
gmsh.write('example_02.vtk')
...
#Exemplo 4
gmsh.initialize()
gmsh.model.add('example 4')
gmsh.model.occ.addRectangle(0, 0, 0, 1, 1, tag=20)
gmsh.model.occ.addDisk(0.5, 0.5, 0, 0.3, 0.3, tag=21)

gmsh.model.occ.cut([(2, 20)], [(2, 21)], tag=40, removeObject=True, removeTool=True)

gmsh.model.occ.synchronize()
gmsh.model.addPhysicalGroup(2, [40])
gmsh.model.mesh.generate(2)
gmsh.model.mesh.refine()


edges = gmsh.model.getBoundary([(2,40)], oriented=False, recursive=False)
print('Edges:', edges)
e1 = edges[0][1]

b1 = gmsh.model.addPhysicalGroup(1, [e1], tag=1)
gmsh.model.setPhysicalName(1, b1, name="edge1")
...
types, elementTags, nodeTagsPerElement = gmsh.model.mesh.getElements(dim=2)

for t, elems, conn in zip(types, elementTags, nodeTagsPerElement):
    elemTypeName = gmsh.model.mesh.getElementProperties(t)[0]
    print(f"\nElement type: {elemTypeName}")
    print(f"Number of elements: {len(elems)}")
    numNodesPerElem = len(conn) // len(elems)
    print(f"Nodes per element: {numNodesPerElem}")
    connectivity = np.array(conn).reshape(-1, numNodesPerElem)

gmsh.write('example_04.msh')
gmsh.write('example_04.vtk')
gmsh.finalize()

problem = fem2dht()

mesh = meshio.read('example_04.msh')
meshio.write("example_04.vtu", mesh)
...
#Boudary conditions
nodes = mesh.cell_sets['edge1'][0]
values = np.ones(len(nodes)) * 20
...
#Exemplo 7
gmsh.initialize()
gmsh.model.add('Example 7')
gmsh.model.occ.addRectangle(0, 0, 0, 1, 1, tag=20)
gmsh.model.occ.addPoint(0.5, 0.5, 0, tag=10)
gmsh.model.occ.addPoint(0.5, 0.9, 0, tag=11)
gmsh.model.occ.addLine(10, 11, tag=12)

gmsh.model.occ.synchronize()
gmsh.model.mesh.embed(1, [12], 2, 20)
gmsh.model.occ.synchronize()
gmsh.model.addPhysicalGroup(2, [20])

gmsh.model.mesh.generate(2)
gmsh.model.mesh.refine()
gmsh.write('example_07.msh')
gmsh.write('example_07.vtk')
...
mesh = meshio.read("example_07.msh")
meshio.write("example_07.vtu", mesh)
'''